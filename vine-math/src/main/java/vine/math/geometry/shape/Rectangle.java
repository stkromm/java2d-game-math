...
public class Rectangle implements Shape, Serializable
{
	...
	/**
	 * Creates a new rectangle defined by the given lower left and right and upper left corner.
	 */
	public Rectangle(
			final float x1,
			final float y1,
			...
			final float y3)
	{
		...
		upperLeftX = x3;
		upperLeftY = y3;
		height = VectorUtils.length(x3 - x1, y3 - y1);
		width = VectorUtils.length(x2 - x1, y2 - y1);
	}
	...
	{
		return super.toString() + " lowerLeftCorner:(" + lowerLeftX + "," + lowerLeftY
				+ "),lowerRightCorner:(" + lowerRightX + "," + lowerRightY + "),upperLeftCorner:("
	...
	public final boolean equals(final Object object)
	{
		...
		return rectangle.lowerLeftX == lowerLeftX && rectangle.lowerLeftY == lowerLeftY
				...
				&& rectangle.upperLeftX == upperLeftX && rectangle.upperLeftY == upperLeftY
